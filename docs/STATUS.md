...
- Storefront and API are deployed on Vercel with Neon PostgreSQL; see VERCEL.md for public URLs and verified checks. The admin login page and verified-Google owner provisioning code are hosted; the authorized admin domain and production owner allowlist are configured; real owner sign-in verification remains in progress. The delivery app and Redis/BullMQ worker are not hosted. Production identity and payment setup remain incomplete; no real payment was performed.
